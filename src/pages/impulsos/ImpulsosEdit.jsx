...
export const ImpulsosEdit = () => {
  ...
  useEffect(() => {
    const fetchImpulso = async () => {
      try {
        ...
        const impulso = response.data;

        if (!impulso) {
          toast({
            title: "Erro ao carregar impulso",
            description: "Impulso não encontrado",
            variant: "destructive",
          });
          navigate("/impulsos");
          return;
        ...
        toast({
          title: "Erro ao carregar impulso",
          description: "Não foi possível carregar os dados do impulso",
          variant: "destructive",
        });
        navigate("/impulsos");
      } finally {
  ...
  const handleSubmit = async (data) => {
    ...
    try {
      ...
      toast({
        title: "Impulso atualizado com sucesso.",
        variant: "success",
      });
      ...
      toast({
        title: "Erro ao atualizar o impulso",
      ...
      if (error.response?.status === 403) {
        form.setError("root", {
          type: "manual",
          message:
            "Você não tem permissão para alterar impulsos para outras empresas.",
        });
      }
    } finally {
